...
const DashboardHeader = ({
  ...
  dashboardData,
  overallStats,
  onLogout // <--- New Prop received here
}) => {
    ...
    <View>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.idBox}>
          ...
          <TextInput
            style={[styles.idInput, { color: '#666' }]}
            value={householdId || 'Loading...'}
            editable={false}
          />
        </View>
      </View>
        ...
        <View style={styles.simCard}>
          ...
          <View style={styles.simRow}>
              <View style={styles.simCol}>
                  ...
                  <Text style={[styles.simSub, {color: '#ef6c00'}]}>
...
const WasteDashboard = ({ route, navigation }) => {
  ...
  // --- LOGOUT FUNCTION ---
  const handleLogout = () => {
    // replace ensures the user can't press 'back' to return to the dashboard
  ...
  const fetchLinkedHousehold = async () => {
    ...
    // Ensure we use the exact same format as the DB
    const cleanEmail = email.trim().toLowerCase();

    try {
      const res = await axios.get(`${API_BASE}/household/check_by_email/${cleanEmail}`);

      if (res.data.exists && res.data.household_id) {
        setHouseholdId(res.data.household_id);
      } else {
        // This is where your error is happening.
        // If the API returns {exists: false}, the app shows the alert.
        console.log("API could not find household for:", cleanEmail);
      }
    } catch (error) {
  ...
  const fetchOverallStats = async () => {
    try {
      ...
      const sortedStats = Object.values(groups).sort((a, b) => b.sortKey.localeCompare(a.sortKey));
...
const styles = StyleSheet.create({
  ...
  welcomeText: { fontSize: 16, fontWeight: 'bold', color: '#102a43' },
  logoutBtn: { backgroundColor: '#ffebee', paddingVertical: 6, paddingHorizontal: 12, borderRadius: 6 },
